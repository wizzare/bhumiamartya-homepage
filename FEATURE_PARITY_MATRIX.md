...
| Community page (`/komunitas/`) | **EXISTING (Phase 2, 2026-09-25)** | `komunitas/index.html` built: hero, Sobat Mistis, Program Komunitas (Ngopi Ilmu/Ngopi Rasa/Rabu Bertumbuh/Kelas Sobat Mistis), Community Journey flow, evergreen "Kegiatan di Bhumi" activity cards (no fabricated dates/status), community guidelines, final CTA. WebPage + BreadcrumbList structured data, unique title/meta/canonical. No event CMS, no invented social proof/member counts |
| Homepage community CTA → `/komunitas/` | **EXISTING (Phase 2)** | Community section's "Gabung Komunitas Bhumi" primary CTA in `index.html` now links to `/komunitas/` (was `/contact/` as a Phase 1 temporary mapping). Hero's "Gabung Komunitas" secondary CTA unchanged, still `#komunitas` per the preferred mapping |
...
| Claim governance / content compliance (about) | **FIXED (Phase 2)** | `about/index.html`'s "Pendekatan Holistik" (same "metode ilmiah ... metafisika" phrase as the homepage) and "Berbasis Ilmiah" ("literatur psikologi dan astrologi yang teruji") cards revised to "Pendekatan Multidisiplin" and "Metodologi Transparan" per `docs/CONTENT_GOVERNANCE.md`. Rest of `about/index.html` (vision/mission copy, team bios) not touched — no other governance violations found there |
| Business location: Nusa Dua, Bali, Indonesia | **EXISTING — repository-wide (Location Consistency Patch, 2026-09-25)** | Fixed in `index.html` (visible copy + founder section + footer + `PostalAddress` JSON-LD), `about/index.html`, `contact/index.html` (Phase 1), and — in this follow-up patch — `privacy-policy/index.html`, `terms/index.html`, `produk/blueprint-reading/index.html`, `kenali-diri/human-design/index.html`, `kenali-diri/mbti/index.html`. Repository-wide grep for "Jakarta Timur" / "DKI Jakarta" now returns zero results. `weton/index.html` and `ebook/index.html` were already correct beforehand |
...
| Consent architecture v2 (Analytics vs. Marketing & Ads) | **EXISTING (Ads & Tracking Foundation, 2026-09-25)** | `assets/bhumi-consent.js` rewritten with independent Necessary/Analytics/Marketing & Ads categories, `bhumi_consent_v2` storage, privacy-safe v1 migration. Fixed audit-found bug: analytics consent no longer silently grants marketing consent. See `docs/ADS_TRACKING_READINESS.md` §1 |
| Google Analytics 4 consent gating | **EXISTING (Ads & Tracking Foundation)** | `assets/bhumi-analytics.js`: GA4 (`G-BLNCYH2290`) loads only after Analytics consent, independent of Marketing & Ads. `docs/ADS_TRACKING_READINESS.md` §2 |
| Meta Pixel consent gating | **EXISTING (Ads & Tracking Foundation)** | Meta Pixel (`392010474954002`) loads only after Marketing & Ads consent, independent of Analytics. `docs/ADS_TRACKING_READINESS.md` §3 |
| Google AdSense consent gating (Auto Ads readiness) | **FIXED (Ads & Tracking Foundation)** | Audit found `assets/bhumi-adsense.js` loading unconditionally with zero consent gating; fixed to require Marketing & Ads consent. Auto Ads readiness only, no ad units declared. `docs/ADS_TRACKING_READINESS.md` §4 |
| Google-certified CMP / IAB TCF status | **NOT CLAIMED — documented boundary** | Repository's consent banner is a first-party mechanism only; certified-CMP/TCF compliance for EEA/UK/Switzerland is an external AdSense-account decision, not implemented in code. `docs/ADS_TRACKING_READINESS.md` §4 |
| ChatGPT Ads crawler allowlist (`robots.txt`) | **EXISTING (Ads & Tracking Foundation)** | `OAI-AdsBot` and `OAI-SearchBot` added to `robots.txt`; `GPTBot` intentionally not added (no product decision). `docs/ADS_TRACKING_READINESS.md` §5 |
| `community_join_click` / `personal_blueprint_click` events | **EXISTING (Ads & Tracking Foundation)** | Added to the tracking event model in `index.html` and `komunitas/index.html` CTA handlers. `docs/ADS_TRACKING_READINESS.md` §6 |
| Ads & Tracking external dashboard config (AdSense Auto Ads, Meta Events Manager, ChatGPT Ads Manager) | **NOT DONE — external, out of repository scope** | See `docs/ADS_TRACKING_READINESS.md` §7 for the full remaining checklist. Not claimed complete by this repository |
