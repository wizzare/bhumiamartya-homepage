...
// Applied to already-escaped text: `**` delimiters are plain ASCII and untouched by esc(),
// so this is safe to run after escaping without opening any injection risk.
...
export function renderArticleBody(content) {
  ...
  return blocks.map((block) => {
    ...
    if (/^##\s+/.test(trimmed)) return `<h2>${boldify(esc(trimmed.replace(/^##\s+/, '')))}</h2>`;
    if (/^###\s+/.test(trimmed)) return `<h3>${boldify(esc(trimmed.replace(/^###\s+/, '')))}</h3>`;
    if (/^#\s+/.test(trimmed)) return `<h2>${boldify(esc(trimmed.replace(/^#\s+/, '')))}</h2>`;
    if (/^-\s+/.test(trimmed)) {
      // Trim each line individually first: a multi-line list block can have interior lines
      // that keep their own leading indentation (e.g. from a Google Doc export), which would
      // otherwise fail the "^-\s+" test and silently drop that bullet from the rendered page.
      const items = trimmed.split('\n').map((l) => l.trim()).filter((l) => /^-\s+/.test(l))
        .map((l) => `<li>${boldify(esc(l.replace(/^-\s+/, '')))}</li>`).join('');
      return `<p>${items}</p>`;
    }
