...
import { find as findGeoTz } from "geo-tz/all";
...
function resolveIanaTimezone(timezone, latitude, longitude) {
  // 1. Valid explicit IANA timezone
  if (timezone && typeof timezone === "string") {
    ...
    if (lower === "asia/jakarta" || lower === "wib") return "Asia/Jakarta";
    if (lower === "asia/makassar" || lower === "wita") return "Asia/Makassar";
    if (lower === "asia/jayapura" || lower === "wit") return "Asia/Jayapura";
    if (lower === "utc" || lower === "gmt") return "UTC";
  }

  // 3. Geographic timezone from latitude + longitude
  if (
    typeof latitude === "number" && Number.isFinite(latitude) &&
    typeof longitude === "number" && Number.isFinite(longitude)
  ) {
    try {
      const candidates = findGeoTz(latitude, longitude);
      if (Array.isArray(candidates) && candidates.length > 0) {
        // Use first valid timezone returned by geo-tz
        // Note: Border coordinates may return multiple candidate zones
        return candidates[0];
      }
    } catch (e) {}
  }
...
function parseNumericOffsetMinutes(timezone) {
  if (!timezone || typeof timezone !== "string") return null;
  const m = timezone.trim().match(/^([+-])(\d{1,2})(?::?(\d{2}))?$/);
  if (!m) return null;
  const sign = m[1] === "-" ? -1 : 1;
  return sign * (Number(m[2]) * 60 + Number(m[3] || "0"));
}

function getUtcFromLocalTime(birthDate, birthTime, ianaTimezone) {
  const [y, m, d] = birthDate.split("-").map(Number);
  const t = (birthTime || "12:00").length === 5 ? `${birthTime}:00` : (birthTime || "12:00:00");
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const p = {};
    parts.forEach(part => { p[part.type] = Number(part.value); });
    if (p.hour === 24) p.hour = 0;

    const formattedUtcMs = Date.UTC(p.year, p.month - 1, p.day, p.hour, p.minute, p.second);
...
function birthToUtc(birthDate, birthTime, timezone, latitude, longitude) {
  // 1. Valid explicit IANA timezone OR 3. Geographic timezone from lat/lon
  const ianaZone = resolveIanaTimezone(timezone, latitude, longitude);
  if (ianaZone) {
    try {
      return getUtcFromLocalTime(birthDate, birthTime, ianaZone);
    } catch (e) {}
  }

  // 2. Valid explicit numeric UTC offset (e.g. +07:00, -05:00)
  const numericOffset = parseNumericOffsetMinutes(timezone);
  const t = (birthTime || "12:00").length === 5 ? birthTime + ":00" : (birthTime || "12:00");
  const [y, m, d] = birthDate.split("-").map(Number);
  const [hh, mm, ss = 0] = t.split(":").map(Number);

  if (numericOffset !== null) {
    return new Date(Date.UTC(y, m - 1, d, hh, mm, ss) - numericOffset * 60000);
  }

  // 4. Last-resort legacy fallback: longitude approximation
  if (typeof longitude === "number" && Number.isFinite(longitude)) {
    const approxOffsetMin = Math.round(longitude / 15) * 60;
    return new Date(Date.UTC(y, m - 1, d, hh, mm, ss) - approxOffsetMin * 60000);
  }
...
function getNorthNodeLongitude(date) {
  const stepMs = 0.05 * 86_400_000;
  const prev = moonCartesianEcliptic(new Date(date.getTime() - stepMs));
  const now = moonCartesianEcliptic(date);
  const next = moonCartesianEcliptic(new Date(date.getTime() + stepMs));
  const vx = next[0] - prev[0];
  const vy = next[1] - prev[1];
  const vz = next[2] - prev[2];
  const hx = now[1] * vz - now[2] * vy;
  const hy = now[2] * vx - now[0] * vz;
...
export function calculateHumanDesign({ birthDate, birthTime, timezone, birthUtc: providedUtc, latitude, longitude }) {
  try {
    const birthUtc = providedUtc instanceof Date && !isNaN(providedUtc.getTime())
      ? providedUtc
      : birthToUtc(birthDate, birthTime, timezone, latitude, longitude);
