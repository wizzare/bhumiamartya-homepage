...
function baseFromLongitude(lon) {
  ...
  return Math.floor((colorPos % 1) * 5) + 1;
}
...
function findDesignDate(birthUtc) {
  ...
  let older = new Date(birthUtc.getTime() - 100 * 86400000);
  let newer = new Date(birthUtc.getTime() - 70 * 86400000);
  for (let i = 0; i < 32; i++) {
    const mid = new Date((older.getTime() + newer.getTime()) / 2);
    const midLon = eclipticLongitude(Astronomy.Body.Sun, mid);
    const travel = normalizeDegrees(pSunLon - midLon);
    if (travel > 88) {
      older = mid;
    } else {
      newer = mid;
  ...
  return new Date((older.getTime() + newer.getTime()) / 2);
}
...
export function calculateHumanDesign({ birthDate, birthTime, timezone, birthUtc: providedUtc }) {
  try {
    const birthUtc = providedUtc instanceof Date && !isNaN(providedUtc.getTime())
      ? providedUtc
      : birthToUtc(birthDate, birthTime, timezone);
