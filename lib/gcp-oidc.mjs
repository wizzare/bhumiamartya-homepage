...
export function decodeClaimsFromToken(token) {
  if (!token) return {};
...
export function logSanitizedClaims(token, tag = 'sts') {
  const c = decodeClaimsFromToken(token);
  console.log(`[gcp-oidc-claims] tag=${tag} owner="${c.owner}" project="${c.project}" project_id="${c.project_id}" environment="${c.environment}" sub="${c.sub}" aud="${c.aud}" iss="${c.iss}"`);
}
    ...
    subject_token_supplier: {
      ...
      async getSubjectToken() {
        try {
          ...
          logSanitizedClaims(token, 'create');
          return token;
        } catch (err) {
          logSanitizedAuthError(err);
          throw err;
    ...
    subject_token_supplier: {
      ...
      async getSubjectToken() {
        try {
          ...
          logSanitizedClaims(token, 'fetch');
          return token;
        } catch (err) {
          logSanitizedAuthError(err);
          throw err;
