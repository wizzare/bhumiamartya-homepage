...
export function logSanitizedAuthErrorStage(err) {
  const status = err?.response?.status ?? 'N/A';
  const safeError = err?.response?.data?.error ?? err?.code ?? err?.name ?? 'UNKNOWN';
  console.error(`[gcp-oidc] stage=sts status=${status} error=${String(safeError).replace(/[^A-Za-z0-9_.-]/g, '')}`);
}
...
function wrapAuthClientDiagnostics(authClient) {
  ...
  wrapped.getAccessToken = async function getAccessTokenDiagnostic(...args) {
    try {
      ...
      logSanitizedAuthErrorStage(err);
      throw err;
    }
  };
