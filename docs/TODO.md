...
- [x] **(Location Consistency Patch, 2026-09-25)** Fixed remaining "Jakarta Timur, DKI Jakarta, Indonesia" business-location references in `privacy-policy/index.html` (2 occurrences) and `terms/index.html` (1 occurrence) to "Nusa Dua, Bali, Indonesia". A repository-wide audit also found and fixed the same stale location ("Lokasi: Jakarta, Indonesia") in the footers of `produk/blueprint-reading/index.html`, `kenali-diri/human-design/index.html`, and `kenali-diri/mbti/index.html`. Repository-wide grep for "Jakarta Timur" / "DKI Jakarta" now returns zero results outside this document's own history. `weton/index.html` and `ebook/index.html` were already correct (Nusa Dua) before this patch.
- [x] **(Phase 2, 2026-09-25)** About Claim Cleanup: revised `about/index.html`'s "Pendekatan Holistik" and "Berbasis Ilmiah" cards (same overbroad "metode ilmiah"/"literatur ... teruji" framing already fixed on the homepage in Phase 1) to "Pendekatan Multidisiplin" and "Metodologi Transparan" per `CONTENT_GOVERNANCE.md` §10.
- [ ] **(new, found during Phase 1, still open)** Decide the actual join destination for "Gabung Komunitas Bhumi" — no canonical WhatsApp-group/Discord/Telegram community link exists anywhere in the repository (see `COMMUNITY.md` §10). `/komunitas/` (Phase 2) and the homepage community CTA both still use `/contact/` as the temporary fallback. Replace once a real destination exists.
...
- [x] **(Phase 2, 2026-09-25)** Build `/komunitas/` page per `COMMUNITY.md` §7 — `komunitas/index.html` built: hero, Sobat Mistis, Program Komunitas, Community Journey, evergreen Kegiatan di Bhumi (no fabricated events), community guidelines, final CTA. WebPage + BreadcrumbList structured data.
- [x] **(Phase 2)** Point the homepage community section's primary CTA ("Gabung Komunitas Bhumi") to `/komunitas/` instead of the Phase 1 `/contact/` fallback. The hero's secondary CTA stays `#komunitas` per the preferred mapping.
- [ ] Build Ngopi Rasa as its own dedicated page/content beyond the `/komunitas/` program card — still no standalone page.
- [ ] Build dedicated Rabu Bertumbuh and Kelas Sobat Mistis content beyond the `/komunitas/` program/activity cards — still no standalone page, no schedule, no pricing (none available in the repo).
...
- [ ] Design the `/komunitas/` join-destination integration (WhatsApp/Discord/Telegram/other — not yet decided, see `COMMUNITY.md` §10). `/komunitas/` (Phase 2) exists but still routes "Gabung Komunitas Bhumi" to `/contact/` as a temporary fallback.
